...
st.markdown("Upload your throw-level CSV (e.g., '2025-05-02-MAD-HTX.csv') to get started.")
...
# Filter out empty receiver rows (for completions/targets)
...
with tab1:
    ...
    st.bar_chart(thrower_atts.sort_values(ascending=False).head(15))

    st.subheader("Throwing Accuracy (Completion % by Player)")
    st.bar_chart(thrower_pct.head(15))
...
with tab2:
    ...
    ax.set_xlabel("X (field width)")
    ax.set_ylabel("Y (field length)")
    st.pyplot(fig)
    ...
    ax2.set_xlabel("X (field width)")
    ax2.set_ylabel("Y (field length)")
    st.pyplot(fig2)
    ...
    if not player_throws.empty:
        ...
        ax_pthrow_rel.set_xlabel("Relative X (to throw start)")
        ax_pthrow_rel.set_ylabel("Relative Y (to throw start)")
    ...
    if not player_catches.empty:
        ...
        ax_pcatch_rel.set_xlabel("Relative X (to throw start)")
        ax_pcatch_rel.set_ylabel("Relative Y (to throw start)")
    ...
    if not player_throws.empty:
        ...
        ax.set_xlabel("Relative X (to throw start)")
        ax.set_ylabel("Relative Y (to throw start)")
        ax.legend()
        st.pyplot(fig)
    else:
        st.info(f"No throws recorded for {selected_player}.")
    ...
    st.plotly_chart(px.histogram(df, x='distance', nbins=30, title="Throw Distance Distribution"))
...
with tab3:
    ...
    st.plotly_chart(px.histogram(df, x='result', color='result', title="Throw Outcomes"))

    st.subheader("Common Throw Lengths Leading to Turnovers")
    df_turnover = df[df['result'] != 'Completion']
    st.plotly_chart(px.histogram(df_turnover, x='distance', nbins=20, title="Turnover Throw Distances"))
